...
beforeEach(() => {
  const user = {
    token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ…NWDcDpNwGvlg1_ZxE',
    username: 'why',
    name: 'why'
  }
...
test('renders content appropriately', () => {
  ...
  const { container } = render(
    <Blogs blogs={blogs} addLikes={mockHandler} removeBlog={mockHandler2} />
  )
  const div = container.querySelector('.blogg')

  expect(div).toHaveTextContent('slim shady')
  expect(div).toHaveTextContent('my name is')
  expect(div).not.toHaveTextContent('no url')
...
test('renders content appropriately when button is clicked', async () => {
  ...
  expect(div).toHaveTextContent('slim shady')
  expect(div).toHaveTextContent('my name is')
  expect(div).toHaveTextContent('no url')
